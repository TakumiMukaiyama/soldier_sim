...
class AgentState(BaseModel):
    ...
    energy: float = Field(
        ...
        ge=0.0,
        le=1.0,
        description="Current energy level (1.0=full energy, 0.0=exhausted)",
    )
    social: float = Field(
        ...,
        ge=0.0,
        le=1.0,
        description="Social need level (1.0=satisfied, 0.0=lonely/isolated)",
    )
    hunger: float = Field(
        ..., ge=0.0, le=1.0, description="Hunger level (0.0=well-fed, 1.0=very hungry)"
    )

    # Skill parameters (0.0-1.0)
    weapon_strength: float = Field(
        ..., ge=0.0, le=1.0, description="Weapon handling and combat skill"
    )
    management_skill: float = Field(
        ..., ge=0.0, le=1.0, description="Leadership and management capability"
    )
    sociability: float = Field(
        ..., ge=0.0, le=1.0, description="Communication skills and social competence"
    )
...
class ReflectionOutput(BaseModel):
    ...
    sociability_development: str = Field(
        ..., description="Assessment of communication and social skill development"
    )
